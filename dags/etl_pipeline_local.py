...
# Get local MySQL server username and password from docker environment
mysql_user = os.environ.get("MYSQL_USER")
mysql_password = os.environ.get("MYSQL_PASSWORD")

# Get AWS MySQL server username and password from docker environment
...
def etl_pipeline():
    ...
    def extract():
        ...
        channel_df.to_csv("channel_df_extracted.csv")
        ...
        # Save pandas DataFrame as CSV file
        videos_df.to_csv("videos_df_extracted.csv")  
        ...
        # Save pandas DataFrame as CSV file
        comments_df.to_csv("comments_df_extracted.csv")  
    ...
    def transform():
        ...
        videos_df = pd.read_csv("videos_df_extracted.csv")
        
        # Convert video duration 
        videos_df["video_duration"] = videos_df["video_duration"].apply(convert_iso8601_duration)
        
        # Save pandas DataFrame as CSV file
        videos_df.to_csv("videos_df_transformed.csv")

    # Load data from Pandas DataFrames into local MySQL tables
    @task 
    def load():
        # Load CSV files into pandas DataFrames
        channel_df = pd.read_csv("channel_df_extracted.csv")
        videos_df = pd.read_csv("videos_df_transformed.csv")
        comments_df = pd.read_csv("comments_df_extracted.csv")
        ...
        try:
            ...
            engine = create_engine(f"mysql+mysqlconnector://{mysql_user}:{mysql_password}@host.docker.internal:3306/youtube_analytics") 
        ...
        try:
            # Create an SQLAlchemy engine for interacting with the MySQL database
            engine = create_engine(f"mysql+mysqlconnector://{aws_mysql_user}:{aws_mysql_password}@host.docker.internal:3308/youtube_analytics") 
